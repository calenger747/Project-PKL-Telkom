<?php
	...
	if(isset($_POST['simpan'])) {
		...
		$p_no_ktp	 	= $_POST['no_ktp'];	
		$p_no_kk	 	= $_POST['no_kk'];
		$p_no_sim_a 	= $_POST['no_sim_a'];	
		$p_no_sim_c		= $_POST['no_sim_c'];
		$p_no_bpjs_kes 	= $_POST['no_bpjs_kes'];	
		$p_no_bpjs_tk 	= $_POST['no_bpjs_tk'];

		$q_edit_pegawai 	= mysqli_query($connect, "UPDATE tbl_pegawai SET nama = '$p_nama', id_lahir = '$p_id_lahir', id_alamat = '$p_id_alamat', id_jabatan = '$p_id_jabatan', id_telp = '$p_id_telp', id_status = '$p_id_status', id_tabungan = '$p_id_tabungan', id_dokumen = '$p_id_dokumen' WHERE nik = '$p_nip' ");
		...
		<?php 
				...
				<script type="text/javascript">
					window.location="../?view=profil-pribadi&id=997386798hupa&name=pegaaplication&detailPegawai&nip=<?php echo $p_nip; ?>&status=4";
		        </script>
			<?php
			}
